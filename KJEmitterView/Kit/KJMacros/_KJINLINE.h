...
/// 子线程
NS_INLINE void kGCD_queue(dispatch_block_t block) {
...
/// 延时执行
NS_INLINE void kGCD_after(NSInteger time, dispatch_block_t block) {
    dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_time_t xtime = dispatch_time(DISPATCH_TIME_NOW, time * NSEC_PER_SEC);
    dispatch_after(xtime, queue, block);
}
